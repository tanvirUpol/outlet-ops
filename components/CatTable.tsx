...
const BasicTable: React.FC<Props> = ({ data, category, type }) => {
    ...
    return (
        <div className="mb-4   rounded auto overflow-scroll  shadow   relative max-h-[50dvh]" >
            <table className=" table-fixed w-[1090px] md:w-full shadow-sm rounded ">
                <thead className="sticky top-0" >
                    <tr className="bg-slate-800" >
                        ...
                        <th className="px-4 py-4 text-left text-xs font-medium uppercase tracking-wider text-white cursor-pointer" onClick={() => toggleSort(type + "_this")}>Sales Cont.{sortBy === `${type}_this` &&
                            (sortOrder === "asc" ? "▲" : " ▼")}</th>
                        <th className="px-4 py-4 text-left text-xs font-medium uppercase tracking-wider text-white cursor-pointer" onClick={() => toggleSort(type + "_this")}> Bench Sales Cont. {sortBy === `${type}_this` &&
                            (sortOrder === "asc" ? "▲" : " ▼")}</th>
